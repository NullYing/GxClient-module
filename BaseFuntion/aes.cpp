...
static unsigned char aes_key[AES_KEY_SIZE] = "gxgkteam";
static const unsigned char aes_iv[16] = {0};
#ifdef WIN32
...
{	
	aes_context context;
	unsigned  char plain[MAX_TEXT_LEN] = {0};
	unsigned  char cipher[MAX_TEXT_LEN];
	unsigned char iv[16];
	static unsigned  char cipher_hex[(MAX_TEXT_LEN)*2+1];
	int i;
	char t[3];
	int len = strlen(plaintext);
	if(len > MAX_TEXT_LEN) return NULL;
	memset(plain, 0, MAX_TEXT_LEN);
	memset(cipher, 0, MAX_TEXT_LEN);
	memcpy(iv, aes_iv, 16);
	strncpy((char *)plain, plaintext, MAX_TEXT_LEN);
	
	if(aes_setkey_enc(&context, aes_key, AES_KEY_SIZE) != 0) return NULL;
	aes_crypt_cbc(&context, AES_ENCRYPT, MAX_TEXT_LEN, iv, plain, cipher);	
	
	cipher_hex[0] = 0;
	for(i = 0; i < MAX_TEXT_LEN; i++) {
		snprintf(t, 3, "%02X", cipher[i]); t[2] = 0;		
		strcat((char*)cipher_hex, t);
	}
	cipher_hex[(MAX_TEXT_LEN)*2] = 0;

	return (char*)cipher_hex;
}

BASEFUNTION_API char *aes_decrypt(const char *cipher)
{
	aes_context context;	
	unsigned char cipher_hex[MAX_TEXT_LEN];
	static unsigned char plain[MAX_TEXT_LEN];
	unsigned char iv[16];
	char t[5] = "0x00";
	int i;
	t[4] = 0;
	if(strlen(cipher) != MAX_TEXT_LEN * 2) return NULL;
	memset(cipher_hex, 0, MAX_TEXT_LEN);	
	memcpy(iv, aes_iv, 16);
	for(i = 0; i < MAX_TEXT_LEN; i++) {
		t[2] = cipher[i * 2]; t[3] = cipher[i * 2 + 1]; 		
		cipher_hex[i] = (unsigned char)(strtoul(t, NULL, 16));
	}

	if(aes_setkey_dec(&context, aes_key, AES_KEY_SIZE) != 0) return NULL;
	aes_crypt_cbc(&context, AES_DECRYPT, MAX_TEXT_LEN, iv, cipher_hex, plain);

	plain[MAX_TEXT_LEN-1] = 0;
	return (char*)plain;
}
BASEFUNTION_API void set_aes_key(const char *key)
{
	strncpy((char *)aes_key, key, AES_KEY_SIZE);
}